    ...
    var body: some View {
        VStack(spacing: 2) {
            Text("\(calendar.component(.day, from: date))")
                ...
                .foregroundColor(isToday ? .white : (isSelected ? TaktsTheme.accent : .primary))
                .frame(width: 32, height: 32)
                .background(
                    Circle()
                        .fill(isToday ? TaktsTheme.accent : (isSelected ? TaktsTheme.accent.opacity(0.2) : Color.clear))
                )
...
struct EventRow: View {
    ...
    //     - If deadline exists:
    //       - Small orange dot (8pt circle, TaktsTheme.accent)
